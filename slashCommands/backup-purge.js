...
module.exports = {
    ...
    async execute (config ,client, interaction, serverdata ){
        ...
        getbackup0 = await backupsModel.find({"backup_owner": interaction.member.id});
        const getbackup = getbackup0[0]

        if (!getbackup) {
            return interaction.editReply(' ไม่เจอ Backup ');
        }
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'confirm') {
                ...
                await backupsModel.deleteMany({"backup_owner": interaction.member.id }, function (err) {
